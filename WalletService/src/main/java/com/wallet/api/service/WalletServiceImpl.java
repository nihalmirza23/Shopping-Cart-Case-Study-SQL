...
public class WalletServiceImpl implements EWalletService{
	...
	@Autowired
	private OrderPaymentStatementRepository 
	orderPaymentStatementRepository;
	...
	public EWallet addWallet(EWallet ewallet,int userId, String headers2) {
		...
		headers.add(HttpHeaders.AUTHORIZATION, headers2);
		HttpEntity<String> entity = new HttpEntity<>("some body", headers);
		UserProfile profile=restTemplate.exchange("http://localhost:8095/api/user/"+userId,HttpMethod.GET, entity, UserProfile.class).getBody();
	...
	public EWallet addMoney(int walletId,Double amount) {
	     ...
	     EWallet wallet2=wallet.get();
			...
			/*
			 * if(amount>wallet2.getCurrentBalance()) {
			 * wallet2.setCurrentBalance(wallet2.getCurrentBalance());
			 * 
			 * Statement statement=new Statement();
			 * 
			 * statement.setAmount(0d); statement.setDate(LocalDate.now());
			 * 
			 * 
			 * statement.setTransactionType("DEPOSIT");
			 * statement.setTransactionRemarks(wallet2.getUsername()+" cannot add "
			 * +amount+" at date and time  "+statement.getDate()+" By transaction type" +
			 * " "+statement.getTransactionType()
			 * +". Amount exceeds than current balance. Current balance in wallet is "
			 * +wallet2.getCurrentBalance());
			 * 
			 * 
			 * statementRepo.save(statement);
			 * 
			 ...
			 * 
			 * walletRepo.save(wallet2); return wallet2; }else {
			 */
	...
	/*
	 * //pay money using e-wallet public StatementDto payUsingWallet(String
	 * orderId,String walletId) { Order
	 * order=restTemplate.getForObject("http://localhost:8098/api/allorder/"+
	 * orderId, Order.class); Double amountToPay=order.getAmountPaid();
	 * Optional<EWallet> wallet=walletRepo.findById(walletId); EWallet
	 * wallet2=wallet.get();
	 * if(order.getModeOfPayment().equalsIgnoreCase("Ewallet")) {
	 * 
	 * 
	 * if(amountToPay<wallet2.getCurrentBalance()) {
	 * wallet2.setCurrentBalance(wallet2.getCurrentBalance()-amountToPay);
	 * StatementDto statementDto=new StatementDto();
	 * statementDto.setAmount(amountToPay); statementDto.setDate(LocalDate.now());
	 * statementDto.setOrderId(order.getOrderId());
	 * statementDto.setTransactionType("WITHDRAW");
	 * statementDto.setTransactionRemarks(wallet2.getUsername()+" Payed "
	 * +amountToPay+ " for order id"+order.getOrderId() +
	 * "using ewallet at date "+statementDto.getDate()+" By transaction type "
	 * +statementDto.getTransactionType()); statementDto.setEwallet(wallet2);
	 * 
	 * walletRepo.save(wallet2); return statementDto ;
	 * 
	 * }else { StatementDto statementDto=new StatementDto();
	 * statementDto.setAmount(amountToPay); statementDto.setDate(LocalDate.now());
	 * statementDto.setOrderId(order.getOrderId());
	 * statementDto.setTransactionType("WITHDRAW");
	 * statementDto.setTransactionRemarks(wallet2.getUsername()+" cannot pay "
	 * +amountToPay+ " for order id"+order.getOrderId() +
	 * "using ewallet at date "+statementDto.getDate()+" By transaction type "
	 * +statementDto.getTransactionType()+
	 * " Because current balance is lower than amount to pay");
	 * statementDto.setEwallet(wallet2);
	 * 
	 * walletRepo.save(wallet2); return statementDto; } } return null; }
	 */
	...
	/*
	 * //get statements by wallet Id public List<Statement>
	 * getStatementByWalletId(String walletId) { List<Statement> list =
	 * statementRepo.findByEwalletId(walletId); Collections.reverse(list); return
	 * list; }
	 */
