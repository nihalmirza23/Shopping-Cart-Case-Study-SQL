...
public class PdfService {
	...
	public String generatePdf(int orderId) throws FileNotFoundException {
		...
		UserProfile customerProfile = getuser(order.getUsername());
	//	UserProfile merchaProfile = getuser(order.get());
		items = order.getItems();
		System.out.println(items);
		String path="C:\\Users\\NASLAMMI\\Desktop\\Doc\\Invoice1.pdf";
		...
		titleTable.addCell(new Cell()
						.add(new Paragraph("Sold By : Shopisthan India pvt. ltd.\n")
								.setBold())
...
//		
		...
		buyerTable.addCell(new Cell()
							.add(new Paragraph()
									...
									.add(new Text(String.valueOf(order.getId()))))
							.setBorder(Border.NO_BORDER));	
		...
		for(ItemsAddedInCart item :items) {
			
			Integer quantInt=item.getQuantity();
			String quantity = quantInt.toString(); 
			
			Double AmountDble= item.getProductPrice();
	...
	public Resource getFileAsResource(String fileCode) throws IOException {
        Path dirPath = Paths.get("C:\\Users\\NASLAMMI\\Desktop\\Doc");
